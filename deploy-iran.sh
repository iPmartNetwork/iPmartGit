...
install_npm_deps() {
  ...
  # NPM Mirror registries to try (in order - Iran mirrors first)
  local registries=(
  ...
  if [ "$success" = false ]; then
    ...
    echo ""
    echo -e "  ${ORANGE}Option 1: Set proxy on server${NC}"
    echo -e "    export http_proxy=http://your-proxy:port"
    echo -e "    export https_proxy=http://your-proxy:port"
    echo ""
    echo -e "  ${ORANGE}Option 2: Upload node_modules manually${NC}"
    ...
    echo ""
    echo -e "  ${ORANGE}Option 3: Try with different DNS${NC}"
    ...
    su - "$APP_USER" -c "cd $APP_DIR && npm config set registry https://mirror-npm.runflare.com"
    if su - "$APP_USER" -c "cd $APP_DIR && npm install --production --loglevel=error" 2>&1 | tail -5; then
